...
#SBATCH -J di-chipmunk
#SBATCH -p fat
#SBATCH -t 480:00
#SBATCH -n 4
#SBATCH -N 1
#SBATCH -o ./jobs/di-chipmunk.%J
#SBATCH -C scratch2
...
result_dir=${RESULT_DIR}/ENCODE/di-chipmunk/${bn}

mkdir -p ${result_dir}
 
#echo "Running di-ChIPMunk..."
/usr/bin/time -f'urug%e' java -cp ${CHIPMUNK} ru.autosome.di.ChIPMunk 8 12 yes 1.0 s:${ENCODE_FASTA_DIR}/${fn} 200 20 1 4 > ${result_dir}/${bn}.out 2>&1
...
${FDR} ${result_dir}/ ${ENCODE_FASTA_DIR}/${fn} --BaMMFile ${result_dir}/${bn}.ihbcp --bgModelFile ${result_dir}/${bn}.hbcp -m 10 -k 1 --cvFold 1
