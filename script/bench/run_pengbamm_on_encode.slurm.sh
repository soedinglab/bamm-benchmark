...
#SBATCH -J pengbamm_encode
#SBATCH -p fat
#SBATCH -t 120:00
#SBATCH -n 4
#SBATCH -N 1
#SBATCH -o ./jobs/pengbamm_encode.%J
#SBATCH -C scratch2
#SBATCH -A all
...
result_dir=${RESULT_DIR}/ENCODE/pengbamm/${bn}
mkdir -p ${result_dir}
...
for k in {0,1,2,5}
do
        result_dir=${RESULT_DIR}/ENCODE/bamm_k${k}_mask/${bn}
        mkdir -p ${result_dir}
	/usr/bin/time -f 'urug%e' ${BAMMMOTIF} ${result_dir} ${seq} --PWMFile ${peng} --maxPWM 1 --EM -k ${k} --advanceEM > ${result_dir}/${bn}.logg 2>&1
	t=$(grep "urug*" ${result_dir}/${bn}.logg | sed "s/urug//")
...
for k in {0,1,2,5}
do
        result_dir=${RESULT_DIR}/ENCODE/bamm_k${k}_full/${bn}
        mkdir -p ${result_dir}
	/usr/bin/time -f 'urug%e' ${BAMMMOTIF} ${result_dir} ${seq} --PWMFile ${peng} --maxPWM 1 --EM -k ${k} > ${result_dir}/${bn}.logg 2>&1
	t=$(grep "urug*" ${result_dir}/${bn}.logg | sed "s/urug//")
	total_time=$(awk "BEGIN {print $peng_time+$t; exit}")
	echo "${bn}	${total_time}" > ${result_dir}/${bn}.time
